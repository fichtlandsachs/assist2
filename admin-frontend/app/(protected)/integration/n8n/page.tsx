...
export default function N8nPage() {
  ...
  useEffect(() => {
    fetchConfig().then(c => { setConfig(c); setUrl(c["n8n.url"]?.value ?? ""); }).catch(() => {});
  }, []);

  const isKeySet = config["n8n.api_key"]?.is_set ?? false;

  async function handleSave() {
    setSaving(true);
    try {
      await patchConfig("n8n.url", url || null);
      if (editKey) await patchConfig("n8n.api_key", apiKey);
    ...
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white text-lg font-bold"
          style={{ background: "#059669" }}>n8n</div>
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--ink)" }}>n8n</h1>
          <p className="text-sm" style={{ color: "var(--ink-faint)" }}>Workflow Automation · Docker-Ressource · Integration Layer</p>
        </div>
      </div>
      <div className="neo-card p-5 space-y-4">
        ...
        <div>
          ...
          <input type="text" value={url} onChange={e => setUrl(e.target.value)}
            placeholder="http://heykarl-n8n:5678" className="neo-input w-full text-sm" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1" style={{ color: "var(--ink-mid)" }}>API Key</label>
          {!editKey ? (
            ...
            <input type="password" value={apiKey ?? ""} onChange={e => setApiKey(e.target.value || null)}
              placeholder="API Key" className="neo-input w-full text-sm" />
          )}
        </div>
