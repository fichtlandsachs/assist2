...
function InviteModal({ orgId, roles, onClose, onSuccess }: {
  orgId: string; roles: RoleItem[]; onClose: () => void; onSuccess: () => void;
}) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="w-full max-w-md p-6 rounded-sm border" style={{ background: "var(--card)", borderColor: "var(--paper-rule)" }}>
        ...
        <form onSubmit={e => void handleSubmit(e)} className="space-y-4">
          ...
          {roles.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-[var(--ink-mid)] mb-2">Rollen (optional)</label>
              <div className="flex flex-wrap gap-2">
                {roles.map(r => (
                  <button key={r.id} type="button" onClick={() => toggleRole(r.id)}
                    className={`text-xs px-2 py-1 rounded border transition-colors ${selectedRoles.includes(r.id) ? "bg-[var(--accent-red)] text-white border-[var(--accent-red)]" : "border-[var(--paper-rule)] text-[var(--ink-mid)]"}`}>
                    {r.name}
                  </button>
                ))}
              </div>
          ...
          <div className="flex gap-2 justify-end pt-1">
            ...
            <button type="submit" disabled={saving} className="px-3 py-1.5 text-sm rounded-sm text-white bg-[var(--accent-red)] disabled:opacity-50">{saving ? "…" : "Einladen"}</button>
          </div>
        </form>
      </div>
    </div>
...
export function MembersSection({ orgId }: { orgId: string }) {
  ...
  const updateStatus = useCallback(async (membershipId: string, status: "active" | "suspended") => {
  ...
  const generateInviteLink = async () => {
    const res = await apiRequest<{ url: string }>(`/api/v1/organizations/${orgId}/invite-link`, { method: "POST" });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowInvite(true)}
          ...
          <Plus size={13} /> Mitglied einladen
        </button>
      </div>

      <div className="border border-[var(--paper-rule)] rounded-sm overflow-hidden">
        <div className="grid grid-cols-[2rem_1fr_auto_auto_auto_2.5rem] gap-3 px-4 py-2 text-xs font-medium text-[var(--ink-faint)] border-b border-[var(--paper-rule)]"
          style={{ background: "var(--paper-warm)" }}>
          <input type="checkbox" checked={selected.size === members.length && members.length > 0} onChange={toggleAll} className="mt-0.5" />
          <span>Name / E-Mail</span><span>Rollen</span><span>Status</span><span>Beigetreten</span><span />
        </div>
        {members.length === 0 ? (
          <div className="px-4 py-8 text-center text-sm text-[var(--ink-faint)]">Keine Mitglieder gefunden.</div>
        ) : members.map(m => (
          <div key={m.id} className="grid grid-cols-[2rem_1fr_auto_auto_auto_2.5rem] gap-3 items-center px-4 py-2.5 border-b border-[var(--paper-rule)] last:border-0 hover:bg-[var(--paper-warm)] transition-colors">
            ...
            <div className="relative">
              ...
              {openMenu === m.id && (
                <div className="absolute right-0 top-7 w-44 rounded-sm border shadow-sm z-20 text-sm"
                  style={{ background: "var(--card)", borderColor: "var(--paper-rule)" }}>
                  {m.status === "active" ? (
                    <button onClick={() => { setOpenMenu(null); setConfirm({ message: `${m.user.display_name} suspendieren?`, onConfirm: () => { setConfirm(null); void updateStatus(m.id, "suspended"); } }); }}
                      className="flex items-center gap-2 w-full px-3 py-2 hover:bg-[var(--paper-warm)] text-[var(--ink-mid)]">
                      <UserX size={13} /> Suspendieren
                    </button>
                  ) : (
                    <button onClick={() => { setOpenMenu(null); void updateStatus(m.id, "active"); }}
                      className="flex items-center gap-2 w-full px-3 py-2 hover:bg-[var(--paper-warm)] text-[var(--ink-mid)]">
                      <UserCheck size={13} /> Reaktivieren
                    </button>
                  )}
                  <button onClick={() => { setOpenMenu(null); setConfirm({ message: `${m.user.display_name} aus der Organisation entfernen?`, onConfirm: () => { setConfirm(null); void removeMember(m.id); } }); }}
      ...
      {selected.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 px-4 py-2.5 rounded-sm border shadow-lg z-30"
          style={{ background: "var(--card)", borderColor: "var(--paper-rule)" }}>
          <span className="text-sm text-[var(--ink-mid)]">{selected.size} ausgewählt</span>
          <button onClick={() => void bulkAction("suspended")} className="text-xs px-2.5 py-1.5 rounded border border-[var(--paper-rule)] text-[var(--ink-mid)] hover:bg-[var(--paper-warm)]">Suspendieren</button>
          <button onClick={() => void bulkAction("active")} className="text-xs px-2.5 py-1.5 rounded border border-[var(--paper-rule)] text-[var(--ink-mid)] hover:bg-[var(--paper-warm)]">Reaktivieren</button>
          <button onClick={() => setConfirm({ message: `${selected.size} Mitglieder entfernen?`, onConfirm: () => { setConfirm(null); void bulkAction("remove"); } })}
            className="text-xs px-2.5 py-1.5 rounded text-white bg-red-600">Entfernen</button>
        </div>
      )}

      <div className="border border-[var(--paper-rule)] rounded-sm p-4 space-y-3" style={{ background: "var(--paper-warm)" }}>
        <div className="flex items-center gap-2">
          <Link2 size={14} className="text-[var(--ink-mid)]" />
          <h3 className="text-sm font-semibold text-[var(--ink)]">Einladungslink</h3>
        </div>
        <p className="text-xs text-[var(--ink-faint)]">Generiere einen Link, den neue Mitglieder direkt aufrufen können. Gültig für 24 Stunden.</p>
        {inviteLink ? (
          <div className="flex items-center gap-2">
            <input readOnly value={inviteLink} className="flex-1 px-2 py-1.5 text-xs border border-[var(--paper-rule)] rounded-sm bg-[var(--card)] text-[var(--ink-faint)]" />
            <button onClick={copyLink} className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs border border-[var(--paper-rule)] rounded-sm text-[var(--ink-mid)] hover:bg-[var(--paper-warm)]">
              {copied ? <Check size={12} className="text-[var(--green)]" /> : <Copy size={12} />}
              {copied ? "Kopiert" : "Kopieren"}
            </button>
          </div>
        ) : (
          <button onClick={() => void generateInviteLink()} className="text-xs px-3 py-1.5 rounded-sm border border-[var(--paper-rule)] text-[var(--ink-mid)] hover:bg-[var(--paper-warm)]">Link generieren</button>
        )}
      </div>

      {showInvite && <InviteModal orgId={orgId} roles={roles} onClose={() => setShowInvite(false)} onSuccess={() => void mutate()} />}
